...
class InnValidator implements InnValidatorInterface
{
     ...
     * Алгоритм:
     * 1) Вычисляется контрольная сумма как сумма произведений соответствующего числа в номере на коэффициенты [2, 4, 10, 3, 5, 9, 4, 6, 8, 0]
     ...
     * 5) Если контрольное число(2) больше 9, то контрольное число(2) вычисляется как остаток от деления контрольного числа(2) на 10
     * 6) Контрольное число(1) проверяется с одиннадцатым знаком ИНН и контрольное число(2) проверяется с двенадцатым знаком ИНН
